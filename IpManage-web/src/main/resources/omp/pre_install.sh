...
logfile="uyun-IpManage-install.log"
...
It looks like you hit an issue when trying to install the uyun-IpManage.
...
DISCONF_USER=admin
DISCONF_PASS=admin
DISCONF_UYUN_IpManage_KEY=IpManage.properties
...
ARGS=`getopt -o h:: --long disconf-host:,disconf-port:,disconf-user:,disconf-passwd:,local-ip:,remote-ips:,install-role:,running-user:,help:: -n 'install.sh' -- "$@"`
...
for DISCONF_IP in ${DISCONF_IPS[@]};do
    ...
    curl -b cookies -c cookies --connect-timeout 5 -sS $LOGIN_URL --data "name=$DISCONF_USER&password=$(decrypt $DISCONF_PASS)&remember=0" | grep -v err_no > result
...
#修改企业版参数
sh -c "sed -i 's@bat.developer.mode=true@@' ./IpManage.properties"
...
# 获取configId配置项
curl -b cookies -c cookies -sS "$CONF_LIST_URL?appId=$UYUN_APP_ID&envId=$ENV_ID&version=$DISCONF_VERSION" | grep -v err_no > result
IpManage_CONFIG_ID=$(cat result | ./jq '.page.result[] | select(.key == "IpManage.properties") | .configId')

if [ -n "$IpManage_CONFIG_ID" ]; then
    echo "IpManage.properties is exist. "
else
	IpManage_UPLOAD_URL="$FILE_UPLOAD_URL?appId=$UYUN_APP_ID&envId=$ENV_ID&version=$DISCONF_VERSION"
	
	uploadFile "$IpManage_CONFIG_ID" "IpManage.properties" "$IpManage_UPLOAD_URL"
